import os
from cv2 import resize, CAP_PROP_FPS, CAP_PROP_FRAME_WIDTH, CAP_PROP_FRAME_HEIGHT, CAP_PROP_POS_MSEC, VideoWriter_fourcc, VideoWriter, destroyAllWindows, waitKey
...
class VideoCensor():
    ...
    def calc_image(self, cap):
        ...
        name = os.path.join(tempfile._get_default_tempdir(), str(next(tempfile._get_candidate_names()))+'.mp4')
        output = VideoWriter(name, fourcc, fps, (int(frame_width*scale), int(frame_height*scale)))
