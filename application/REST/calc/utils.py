...
import numpy as np
from cv2 import blur, imread, putText, FONT_HERSHEY_DUPLEX, LINE_AA

IMG_MARGIN = 10  # pixels
IMG_FONT_SIZE = 1
IMG_FONT_THICKNESS = 1
IMG_HANDEDNESS_TEXT_COLOR = (88, 205, 54) # vibrant green
...
def draw_landmarks_on_image(rgb_image, detection_result, text_override=None):
  ...
  for idx in range(len(hand_landmarks_list)):
    ...
    text_y = int(min(y_coordinates) * height) - IMG_MARGIN
    ...
    putText(annotated_image, text,
                (text_x, text_y), FONT_HERSHEY_DUPLEX,
                IMG_FONT_SIZE, IMG_HANDEDNESS_TEXT_COLOR, IMG_FONT_THICKNESS, LINE_AA)
